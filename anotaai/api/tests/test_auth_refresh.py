...
from app.models import Operador, PapelOperador, RefreshToken


@pytest.fixture
def operador(session: Session, id_loja: uuid.UUID) -> Operador:
    operador = Operador(
        id_loja=id_loja,
        nome="Refresh Teste",
...
def test_login_retorna_access_e_refresh_token(client: TestClient, operador: Operador):
    data = _login(client)
...
def test_refresh_emite_novo_par_de_tokens(client: TestClient, operador: Operador):
    limiter.reset()
...
def test_refresh_rotaciona_e_invalida_token_antigo(client: TestClient, operador: Operador):
    limiter.reset()
    tokens = _login(client)
...
def test_refresh_com_token_expirado_retorna_401(client: TestClient, session: Session, operador: Operador):
...
def test_logout_revoga_refresh_token(client: TestClient, operador: Operador):
    limiter.reset()
    tokens = _login(client)
